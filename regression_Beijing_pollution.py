...
def preprocess_data(pro_df, target):
    """
    ...
    """
    df = pro_df.dropna()
    X = pd.get_dummies(df.drop(columns=f'{target}'))
    y = df[f'{target}'].values.reshape(-1, 1)
...
def prod_model_generator(pro_df, target):
    """
    ...
    X_train, X_test, y_train, y_test = preprocess_data(pro_df, target)
